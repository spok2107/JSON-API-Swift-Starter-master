...
internal func exerciseOne() {
    ...
    let postcodeName = userData["results"][0]["location"]["postcode"].intValue
    ...
    print("\(firstName) \(lastName) lives at \(streetName) in \(cityName), \(stateName), \(postcodeName). If you want to contact \(titleName). \(lastName), you can email \(emailAddress) or cell at \(cellPhoneNumber)")
...
print("The following movies are Disney movies:")
    ...
    for movie in allMovies {
        if movie.rightsOwner.containsString("Disnay") {
            print(movie.name)
        }
    }
